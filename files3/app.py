...
@app.route('/api/preview-settings')
def api_preview_settings():
    s = load_settings()
    return jsonify({
        'enabled':    str(s.get('preview_enabled', 'true')).lower() == 'true',
        'duration':   int(s.get('preview_duration', 15)),
        'blur_after': str(s.get('preview_blur_after', 'true')).lower() == 'true',
    })
...
def admin_dashboard():
    ...
    contributors = len(set(n['uploader'] for n in approved))
...
def admin_save_settings():
    ...
    EDITABLE = [
        ...
        'preview_enabled', 'preview_duration', 'preview_blur_after',
    ]
    for key in EDITABLE:
        if key in data:
